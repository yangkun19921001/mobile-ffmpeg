...
static av_cold int mediacodec_encode_init(AVCodecContext* avctx) {
    ...
    hi_logd(avctx, MEDIACODEC_LOG_TAG, "%s %d 时间记录 %s", __FUNCTION__, __LINE__,"mediacodec_encode_init");
...
static void I420toYUV420SemiPlanar(uint8_t* input, int offset, uint8_t* output, int width, int height) {
...
#include <sys/time.h>
static  long long currentTimeMills_();
static  long long currentTimeMills_(){
	struct timeval tv;
...
static int mediacodec_encode_send_frame(AVCodecContext* avctx, const AVFrame* frame) {
    ...
    hi_logd(avctx, MEDIACODEC_LOG_TAG, "%s %d mediacodec_encode_send_frame", __FUNCTION__, __LINE__);
    ...
    uint8_t *nv21 = malloc(bufferSize);


    int ret = av_image_copy_to_buffer(nv21, bufferSize, (const uint8_t **)frame->data,
                            frame->linesize, frame->format,
                            frame->width, frame->height, 1);

    I420toYUV420SemiPlanar(nv21, 0, buffer,frame->width, frame->height);
    ...
    hi_logd(avctx, MEDIACODEC_LOG_TAG, "%s %d (%d %d),mediacodec_encode_send_frame  buffInfo (idx: %u size: %u flags: %d) frameInfo (width: %d height: %d format: %d pts: [%lld %lld] type: %d)", __FUNCTION__, __LINE__,
...
static int mediacodec_encode_receive_packet(AVCodecContext* avctx, AVPacket* pkt) {
    ...
    int bufferIndex = -1;
    int ret = 0;
    long long start_time = currentTimeMills_();
    //do {

       hi_logd(avctx, MEDIACODEC_LOG_TAG, "%s %d 编码记录 %s", __FUNCTION__, __LINE__,"开始编码");

        if (ctx->saw_output_eos) {
            ret = AVERROR_EOF;
            hi_loge(avctx, MEDIACODEC_LOG_TAG, "%s %d ", __FUNCTION__, __LINE__);
          //break;
          int end = currentTimeMills_() - start_time;
              hi_logd(avctx, MEDIACODEC_LOG_TAG, "%s %d 编码记录 %s %d ms", __FUNCTION__, __LINE__,"编码退出",end);
                      return ret;
        }
        ...
        if (bufferIndex < 0) {
            ...
            ret = AVERROR(EAGAIN);
            hi_logd(avctx, MEDIACODEC_LOG_TAG, "%s %d 编码记录 %s", __FUNCTION__, __LINE__,"编码结束");
           //break;
           int end = currentTimeMills_() - start_time;
               hi_logd(avctx, MEDIACODEC_LOG_TAG, "%s %d 编码记录 %s %d ms", __FUNCTION__, __LINE__,"编码退出",end);
                       return ret;
        ...
        //%x 表示16进制输出
        hi_logd(avctx, MEDIACODEC_LOG_TAG, "%s %d PIAO-QUAN-TV AMediaCodec OutputBuffer status: %d outsize: %u flags: %d offset: %d size: %d pts: [%lld %lld] nalu: [%x %x %x %x %x %x] nalu-type: %d",
            __FUNCTION__, __LINE__, bufferIndex, buff_size, bufferInfo.flags, bufferInfo.offset, bufferInfo.size, bufferInfo.presentationTimeUs, pts, data[0], data[1], data[2], data[3], data[4], data[5],data[4] & 0x1f);
...
static av_cold int mediacodec_encode_close(AVCodecContext *avctx) {
    ...
    hi_logd(avctx, MEDIACODEC_LOG_TAG, "%s %d 时间记录 %s", __FUNCTION__, __LINE__,"mediacodec_encode_close");
    if (ctx->codec) {
